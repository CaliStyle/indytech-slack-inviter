...
export default function splash({ path, name, org, coc, logo, active, total, channels, large, iframe }){
  let div = dom('.splash',
    ...
    !iframe && dom('footer',
      'Brought to you by ',
      dom('a href=https://cali-style.com', 'Cali Style Technologies, LLC'),
      dom('br'),
      dom('a class=calistyle-logo href=https://cali-style.com',
        dom(`img src=${path}assets/cst-logo.png`)
      ),
      dom('br'),
      'Powered by ',
...
function style({ logo, active, large, iframe } = {}){
  ...
  css.add('.calistyle-logo', {
    'border': '0'
  });

  css.add('.calistyle-logo img', {
    'max-width': '130px',
    'margin': '1.2em'
  });
